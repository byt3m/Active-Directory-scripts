...
Description: This script will check for inactive user objects in an AD domain. A whitelist can be used.
#>
...
$users = Search-ADaccount -UsersOnly -AccountInactive -Timespan 180 | `
    where-object { $_.SamAccountName -notin $white_list -and $_.LastLogonDate } | `
        Select-Object Name, SamAccountName, DistinguishedName, LastLogonDate, Enabled

$users | Format-Table Name, SamAccountName, LastLogonDate, Enabled


if (ReadInput -msg "Do you want to export it to CSV? (Y/N)"-yesno)
{
    $csv_name = (Get-Date -format "yyyyMMdd") + "_inactive_users.csv"
